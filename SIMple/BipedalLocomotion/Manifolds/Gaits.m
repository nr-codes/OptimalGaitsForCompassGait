...
(* ::Input::Initialization:: *)
Options[BLTangentSpace] = {"P" -> BLP, "nstol" -> Automatic, BLFindBifurcation -> {}};
...
(* ::Input::Initialization:: *)
Options[BLHomotopy] = {"P" -> BLP, BLP -> {}, "H" -> ({{}}&), Man -> {}};

BLHomotopy[cp_, opts:OptionsPattern[]] := BLHomotopy[BLbiped["m[0]"], cp, opts];

BLHomotopy[m_String, cp_, opts:OptionsPattern[]] := 
Module[{f0, g0, R0, H0},
(* root *)
f0 = OptionValue["P"];
(* homotopy *)
...
(* initial values; no homotopy parameter in cp *)
R0 = f0[m, cp, OptionValue[BLP]];
H0 = g0[m, cp, Map -> R0];

(* root function *)
With[{h0 = H0[[1]], f = f0, g = g0}, 
Module[{c, o, \[Lambda], R, H},
c = #1[[;;-2]];
...
H[[1]] = Join[R[[1]], H[[1]] - \[Lambda] h0];
If[Length@H > 1,
H[[2]] = ArrayFlatten[{{R[[2]], 0}, {H[[2]], -{h0}\[Transpose]}}];
];
...
BLHomotopy[{P_, H_}, c_, h_, N_, opts:OptionsPattern[]] := Module[{r, o},
r = BLHomotopy[c, "P" -> P, "H" -> H, opts];

(* add to cm options *)
o = OptionValue[Man];
r = Man[r, Append[c, 1], h, N, o];
r = Select[r, # =!= $Failed&];
Join[{#[[1, 1;;-2]]}, #[[1, -1;;-1]], #[[2;;]]]& /@ r
];
