...
Options[FiniteDifferenceJacobian] = {"p" -> 2, "t" -> "c", "M" -> True};
FiniteDifferenceJacobian[f_, c_, h_:10.0^-5, o:OptionsPattern[]] := Module[{D, I, J},
If[Length@c != Length@h && Length@h > 0, Return[$Failed]];
D = FiniteDifferenceFunction[1, 1, OptionValue["p"], OptionValue["t"]];
...
If[OptionValue["M"],  Transpose[J, RotateRight[Range@Length@Dimensions@J]], J]
];

Hess[d_, t_] := Module[{n, tri},
n = Length@d;
tri = SparseArray[Thread[Subsets[Range@n, {2}] -> t], {n, n}];
DiagonalMatrix[d] + tri + tri\[Transpose]
];

Options[FiniteDifferenceHessian] = {"p" -> 2, "t" -> "c", "M" -> True};
FiniteDifferenceHessian[f_, c_, h_, o:OptionsPattern[]] := Module[{n, DXX, DXY, I, d1, d2, p, t, H},
n = Length@c;
...
!OptionValue["M"], H, 
VectorQ[H[[1]]],Hess@@H,
True, MapThread[Hess, Transpose /@ H]
]
];
